...
public class MessagePController {
    ...
    public int sendMessage(@PathVariable("mess") String mess){
        myMessage.messageout().send(MessageBuilder.withPayload(mess).setHeader("unique", 0).build());
    ...
    public void receive(String mess){
        System.out.println(count2.getAndIncrement()+" "+LocalDateTime.now()+"product---receive()---receive message: "+mess);
    }
}
